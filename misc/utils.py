...
def transform(image, image_size, is_crop, bbox):
  ...
  #  transformed_image = Image.fromarray(image).resize([image_size, image_size],resample=PIL.Image.BICUBIC)
       # scipy.misc.imresize(image, [image_size, image_size], 'bicubic')

    transformed_image = np.array(Image.fromarray(np.uint8(image)).resize((image_size, image_size), Image.BICUBIC))
