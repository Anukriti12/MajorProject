...
import sys
#sys.path.insert(1, '/path/to/how_to_convert_text_to_images')
...
def save_data_list(inpath, outpath, filenames, filename_bbox):
    ...
    for key in filenames:
        ...
        img = utils.get_image(f_name, LOAD_SIZE, is_crop=True, bbox=bbox)
        img = img.astype('uint8')
        hr_images.append(img)
        lr_img = np.array(Image.fromarray(img).resize((lr_size, lr_size),resample=PIL.Image.BICUBIC))
